...
Compilation options: -a arm.cpp -lang cpp -i -os2 -es 1 -single -ftz 0
------------------------------------------------------------ */
...
class mydsp : public one_sample_dsp_real<float> {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a arm.cpp -lang cpp -i -os2 -es 1 -single -ftz 0");
	...
	mydsp() {}
	mydsp(int* icontrol, FAUSTFLOAT* fcontrol, int* izone, float* fzone):one_sample_dsp_real(icontrol, fcontrol, izone, fzone) {}
	...
	virtual void control(int* RESTRICT iControl, FAUSTFLOAT* RESTRICT fControl, int* RESTRICT iZone, float* RESTRICT fZone) {
	...
	virtual int getfZoneSize() { return 0; }
	
	virtual void compute(FAUSTFLOAT* RESTRICT inputs, FAUSTFLOAT* RESTRICT outputs, int* RESTRICT iControl, FAUSTFLOAT* RESTRICT fControl, int* RESTRICT iZone, float* RESTRICT fZone) {
		iVec0[(IOTA & 1)] = 1;
		fRec0[(IOTA & 1)] = (fControl[1] + (fConst2 * fRec0[((IOTA - 1) & 1)]));
		float fTemp0 = fRec0[((IOTA - 0) & 1)];
		float fTemp1 = fRec3[((IOTA - 1) & 1)];
		float fTemp2 = fRec2[((IOTA - 1) & 1)];
		fRec2[(IOTA & 1)] = ((fConst4 * fTemp1) + (fConst5 * fTemp2));
		fRec3[(IOTA & 1)] = ((float((1 - iVec0[((IOTA - 1) & 1)])) + (fConst5 * fTemp1)) - (fConst4 * fTemp2));
		float fTemp3 = fRec4[((IOTA - 1) & 1)];
		float fTemp4 = (fControl[4] + (fTemp3 + -1.0f));
		int iTemp5 = (fTemp4 < 0.0f);
		float fTemp6 = (fControl[4] + fTemp3);
		fRec4[(IOTA & 1)] = (iTemp5 ? fTemp6 : fTemp4);
		float fThen1 = (fControl[4] + (fTemp3 + (fControl[5] * fTemp4)));
		float fRec5 = (iTemp5 ? fTemp6 : fThen1);
		iRec6[(IOTA & 1)] = ((1103515245 * iRec6[((IOTA - 1) & 1)]) + 12345);
		float fTemp7 = fRec1[((IOTA - 2) & 3)];
		fRec7[(IOTA & 1)] = (fControl[7] + (fConst2 * fRec7[((IOTA - 1) & 1)]));
		float fTemp8 = fRec8[((IOTA - 1) & 1)];
		fRec8[(IOTA & 1)] = (fControl[8] + (fTemp8 - std::floor((fControl[8] + fTemp8))));
		float fTemp9 = std::tan((fConst7 * ((0.5f * (fRec7[((IOTA - 0) & 1)] * (((2.0f * (1.0f - std::fabs(((2.0f * fRec8[((IOTA - 0) & 1)]) + -1.0f)))) + -1.0f) + 1.0f))) + 50.0f)));
		float fTemp10 = (1.0f / fTemp9);
		float fTemp11 = fRec1[((IOTA - 1) & 3)];
		float fTemp12 = (((fTemp10 + 0.200000003f) / fTemp9) + 1.0f);
		fRec1[(IOTA & 3)] = (((fControl[2] * (fRec3[((IOTA - 0) & 1)] * ((2.0f * fRec5) + -1.0f))) + (fControl[6] * float(iRec6[((IOTA - 0) & 1)]))) - (((fTemp7 * (((fTemp10 + -0.200000003f) / fTemp9) + 1.0f)) + (2.0f * (fTemp11 * (1.0f - (1.0f / mydsp_faustpower2_f(fTemp9)))))) / fTemp12));
		float fTemp13 = (fTemp7 + (fRec1[((IOTA - 0) & 3)] + (2.0f * fTemp11)));
...
struct ARMController {
	...
	// Copy constants variables in locally defined iZone, fZone
	int* c_iZone = (int*)(ddr_ptr);
	float* c_fZone = (float*)(ddr_ptr + FAUST_INT_CONST);
	
	// Init DSP part (after reset DDR)
        //fDSP->init(SAMPLE_RATE, iZone, fZone);
	// TANG commented following Stephane's comment but not sure
	fDSP->instanceConstantsToMem(SAMPLE_RATE,c_iZone, c_fZone);
	...
	/* 4 - Define UI type using DSP object*/
        ...
        if (SSMSetRegister(VOLUME_SSM)!= XST_SUCCESS) {
