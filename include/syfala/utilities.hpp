...
#define sy_private __attribute__ ((visibility ("internal")))
...
#define sy_verbose(_S, _V, ...)                     \
    do {                                            \
        if (SYFALA_VERBOSE >= _V) {                 \
            sy_print_fn(_S "\r\n", ##__VA_ARGS__);  \
        }                                           \
    } while(0)
 ...
 *   Use sy_debug for debug puprose, such as frame decoding.
 * */
#define sy_printf(_S, ...) sy_verbose(_S, 0, ##__VA_ARGS__)
#define sy_info(_S, ...)   sy_verbose(_S, 1, ##__VA_ARGS__)
#define sy_debug(_S, ...)  sy_verbose(_S, 2, ##__VA_ARGS__)
#define RN(_str) _str "\r\n"
