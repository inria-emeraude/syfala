...
proc get_latency_percentage {latency} {
    if {$::g::multisample > 0} {
        set nsamples $::g::multisample
    } else {
        set nsamples 1
    }
    set tcl_precision 3
    set ncycles [expr $latency/$nsamples]
    set ratio   [expr 48000.0/122885835.0]; # FPGA sys_clk
    return [format {%.2f} [expr $ratio*$ncycles*100.0]]
}
...
#    print_info "Last is interval: $line ($d)"
    ...
    # Append Latency in percentage
    lappend line [get_latency_percentage [lindex $d 1]]
