...
/// configuration for the audio server
#[derive(Debug, Clone)]
...
pub trait AudioServer {
    /// defines new audio server with channel in and channel out
    /// `Tx` is used receive audio from the audio server
    fn new(
        name: &str,
        channel_in: usize,
        channel_out: usize,
    ...
    /// Starts audio server, this needs to be executed to hear something
    fn start(&mut self, config: BufferManagerConfig);
    /// returns the audio server tx to send audio to the network
        ...
        let process = ClosureProcessHandler::new(
            move |_: &jack::Client, ps: &ProcessScope| -> Control {
                ...
                if !out.is_empty() {
                    ...
                    assert_eq!(buffer.len(), out_total_len, "something with the manager.get_adjusted_samples went wrong. The buffer length is not equal to the out length");
                    // deinterleave audio
