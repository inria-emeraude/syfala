...
impl UdpConnection {
    pub async fn new(
        ...
        let (channels_in, channels_out) = {
            ...
            (axi.get_input_channels(), axi.get_output_channels())
        };

        // make ring size a multiple of channels otherwise FPGA crashes
        let aout_size = config.mem_out_max_size - config.mem_out_max_size % channels_out;
        let ain_size = config.mem_in_max_size - config.mem_in_max_size % channels_in;
        tracing::info!(
        ...
        let status = { axi.lock().unwrap().get_status() };
        assert_eq!(
            status, 0,
            "not ok, FPGA not ready, should have status 0 instead it has {}. Check ethernet-ip.cpp for meaning of status bits",
            status
        );
        ...
        loop {
            tokio::select! {
                res = self.socket.recv(&mut buf) => {
                    ...
                    let max_amount_of_samples = calc_max_buffer(self.channels_out, self.max_frame_size);
