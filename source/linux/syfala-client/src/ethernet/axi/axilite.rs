...
use crate::ethernet::axi::{get_hex_from_file, perror};
use crate::ethernet::HLS;
use std::ffi::c_void;
...
// The following makes it possible to share AxiLite between threads.
unsafe impl Send for AxiLite {}

impl AxiLite {
    /// Gets the number of channels the FPGA accepts as *input*.
    pub fn get_input_channels(&mut self) -> usize {
        self.get_u32(HLS::AXI_LITE_AUDIO_IN_CHANNELS) as usize
    }
    /// Gets the number of channels the FPGA accepts as *output*.
    ...
    /// Gets last write index of audio in.
...
impl AxiLite {
    ...
    fn write<T>(&mut self, pos: usize, val: T) {
        let size = size_of::<T>();
        assert!(pos * size < self.size, "Out of bounds!");
    ...
    fn get<T>(&mut self, pos: usize) -> T {
        ...
        let b = self.buffer as *mut T;
        unsafe {
            ptr::read_volatile(b.add(pos / size))
        }
    }
