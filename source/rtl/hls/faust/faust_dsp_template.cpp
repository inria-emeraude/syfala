...
void syfala (
     sy_ap_int audio_in[FAUST_INPUTS],
     sy_ap_int audio_out[FAUST_OUTPUTS],
    ...
    if (arm_ok) {
        if (initialize) {
            ...
            for (int n = 0; n < FAUST_INPUTS; ++n) {
                 inputs[n] = Syfala::HLS::ioreadf(audio_in[n]);
            }
           /* - Update controller values (from ARM) */
            ...
            for (int n = 0; n < FAUST_OUTPUTS; ++n) {
                 float f = clip<sy_real_t>(outputs[n], -1, 1);
