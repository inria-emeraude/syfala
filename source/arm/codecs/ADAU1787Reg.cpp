...
int boot_sequence(int bus, unsigned long codec_addr) {
	if (ADAU17xx::regwrite(bus, codec_addr, REG_CHIP_PWR_IC_1_Sigma_ADDR, 0x11, 0)!= XST_SUCCESS) {
		sy_debug("\033[2K[ADAU1787] Could not initialize boot sequence (reg:REG_CHIP_PWR_IC_1_Sigma_ADDR, value:0x11)\n\r");
		return XST_FAILURE;
	}
	usleep(40000);
	if (ADAU17xx::regwrite(bus, codec_addr, REG_CHIP_PWR_IC_1_Sigma_ADDR, 0x15, 0)!= XST_SUCCESS) {
		sy_debug("\033[2K[ADAU1787] Could not initialize boot sequence (reg:REG_CHIP_PWR_IC_1_Sigma_ADDR, value:0x15)\n\r");
		return XST_FAILURE;
	}
	usleep(40000);
	if (ADAU17xx::regwrite(bus, codec_addr, REG_CHIP_PWR_IC_1_Sigma_ADDR, 0x17, 0)!= XST_SUCCESS) {
		sy_debug("\033[2K[ADAU1787] Could not initialize boot sequence (reg:REG_CHIP_PWR_IC_1_Sigma_ADDR, value:0x17)\n\r");
		return XST_FAILURE;
	}
	return XST_SUCCESS;
}
