...
static void initialize_device(int device) {
     ...
     * then initialize it. */
    if (c == nullptr) {
        Status::warning("[i2c] Could not find XIICPS configuration");
    }
    if (xiicps::initialize_config(&x[device], c, c->BaseAddress) != XST_SUCCESS) {
        Status::warning("[i2c] Could not initialize XIICPS configuration");
    }
    // Perform a self-test to ensure that the hardware was built correctly.
    if (xiicps::self_test(&x[device]) != XST_SUCCESS) {
        Status::warning("[i2c] XIICPS self-test failed, aborting...");
    }
    // Set the I2C serial clock rate.
    if (xiicps::set_sclk(&x[device], IIC_SCLK_RATE) != XST_SUCCESS) {
        Status::warning("[i2c] Failed setting XIICPS serial clock rate, aborting...");
    }
    println("[i2c] XIICPS (device: %d) successfully initialized at %dHz.",
            device, IIC_SCLK_RATE
    );
}
...
void Audio::initialize() {
    ...
    info("[Internal codec] Initialize ADAU1761 on I2C %d at address 0X%02x",IIC_ADAU1761_BUS,IIC_ADAU1761_SLAVE_ADDR_0);
    status = Codecs::ADAU1761::initialize(IIC_ADAU1761_BUS, IIC_ADAU1761_SLAVE_ADDR_0);
#elif SYFALA_BOARD_ZYBO
    info("[Internal codec] Initialize SSM2603 on I2C %d at address 0X%02x",IIC_SSM_BUS,IIC_SSM_SLAVE_ADDR);
    status = Codecs::SSM2603::initialize(IIC_SSM_BUS);
#endif
    if (status != XST_SUCCESS) {
        Status::warning("[audio] Could not initalize on-board device");
    } else {
        println("[audio] Succesfully initialized on-board device");
    }

#if SYFALA_ADAU_EXTERN
    info("[audio] Use external ADAU codecs");
    // Initialize external codecs
    // ADAU1777 (4 possible slots)
    info("[audio] Initialize ADAU1777");
    for (int n = 0; n < 4; ++n) {
         ...
         if ((addr==IIC_MOTHERBOARD_TCA9548A_ADDR || addr==IIC_MOTHERBOARD_PCA9956_ADDR) && SYFALA_ADAU_MOTHERBOARD){
            info("[ADAU1777] WARNING: address conflict! This codec addresse (0X%02x) is already used by the motherboard.\n\r[ADAU1777] Codec 0X%02x will be ignored.",addr, addr);}
         else{
            info("[ADAU1777] Initialize ADAU1777 on I2C %d at address 0X%02x",IIC_ADAU1777_BUS,addr);
            if (Codecs::ADAU1777::initialize(IIC_ADAU1777_BUS, addr) == XST_SUCCESS) {
                adau1777_ndevices += 1;
            }
            else {info("[ADAU1777] Could not initialize codec at address 0X%02x",addr);}
         }
    }
    println("[audio] Found: %d ADAU1777", adau1777_ndevices);//Not a warning or an error if 0 devices detected, maybe 0 ADAU1777 are connected, only ADAU1787 are.

#if SYFALA_ADAU_MOTHERBOARD
    status = XST_SUCCESS;
    info("[audio] Use ADAU1787 Motherboard");
    //motherBoard::searchIIC();

    info("[audio] Initialize LED driver on I2C %d at address 0X%02x",IIC_MOTHERBOARD_PCA9956_BUS,IIC_MOTHERBOARD_PCA9956_ADDR);
    if (LEDdriver::initialize() == XST_SUCCESS) {info("[audio] LED driver detected");}
    else{
        info("[audio] Cannot detect LED driver");
        status=XST_FAILURE;
    }
    info("[audio] Initialize I2C Mux on I2C %d at address 0X%02x",IIC_MOTHERBOARD_TCA9548A_BUS,IIC_MOTHERBOARD_TCA9548A_ADDR);
    if (IICMUX::isConnected() == XST_SUCCESS) {info("[audio] I2C Mux detected");}
    else{
        info("[audio] Cannot detect I2C Mux");
        status=XST_FAILURE;
    }
    info("[audio] Initialize MotherBoard UI");
    if (motherBoard::initializeUI() == XST_SUCCESS) {info("[audio] MotherBoard UI initialized");}
    else{
        info("[audio] MotherBoard UI initialization failed");
        status=XST_FAILURE;
    }

    if(status != XST_SUCCESS) Status::warning("[audio] Could not initalize motherboard, please ensure that you used the correct I2C addresses in ADAU17XX.hpp");
    //Here it's an error: if we declared the motherboard, it must be initialized.
    // ADAU1787
    info("[audio] Initialize ADAU1787");
    for (int muxChannel = 0; muxChannel < 4; ++muxChannel) {
        ...
        for (int codec_nb = 0; codec_nb < 4; ++codec_nb) {
            ...
            if ((addr==IIC_MOTHERBOARD_TCA9548A_ADDR || addr==IIC_MOTHERBOARD_PCA9956_ADDR) && SYFALA_ADAU_MOTHERBOARD){
                info("[ADAU1787] WARNING: address conflict! This codec addresse (0X%02x) is already used by the motherboard.\n\r[ADAU1787] Codec 0X%02x will be ignored.",addr,addr);}
            else{
                debug("\033[2K[ADAU1787] Initialize codec 0x%02x on channel %d \n\r",addr,muxChannel);
                Codecs::ADAU1787::boot_sequence(1, addr);
                if ((Codecs::ADAU1787::initialize(1, addr))==XST_SUCCESS) {
    ...
    for (int n = 0; n < 4; ++n) {
         ...
         debug("\033[2K[ADAU1787] Initialize codec @ 0x%02x \n\r",addr);
    ...
    println("[audio] Found: %d ADAU1787", adau1787_ndevices); //Not a warning or an error if 0 devices detected, maybe 0 ADAU1787 are connected, only ADAU1777 are.
#endif
}
...
int Audio::regwrite(uint8_t* regAddr,int sizeOfRegAddr,uint8_t* value, int sizeOfValue, uint16_t addr, uint8_t bus) {
    ...
    debug("\033[2K[i2c] WRITE: I2C bus=%d, I2C Addr=0x%02x, Reg Addr=0x",bus,addr);
...
int Audio::Codecs::SSM2603::regwrite(uint8_t bus, uint16_t addr, uint16_t data) {
...
int Audio::Codecs::ADAU17xx::regwrite(int bus,
...
int IICMUX::setPort(uint8_t portBits) {
    uint8_t value[1];
    value[0] = portBits;
    return Audio::regwrite(0, 0, value, sizeof(value),
        IIC_MOTHERBOARD_TCA9548A_ADDR,
        IIC_MOTHERBOARD_TCA9548A_BUS
    );
}
...
uint8_t IICMUX::getPort() {
    ...
    uint8_t rx_data[1];
	int status = xiicps::master_recv_polled(&x[IIC_MOTHERBOARD_TCA9548A_BUS],
        rx_data,1, (IIC_MOTHERBOARD_TCA9548A_ADDR & 0xff)
    );
	if (status != XST_SUCCESS) {
		return XST_FAILURE;
	}
...
int IICMUX::isConnected() {
    debug("\033[2K[I2C MUX] Connection test ");
    uint8_t knownValue=123;
    if (IICMUX::setPort(knownValue) != XST_SUCCESS) {
        return XST_FAILURE;
    }
    debug("send %d;", knownValue);
    uint8_t response = getPort();
    debug("recv: %d \n\r", response);
    if (IICMUX::setPort(0x00) != XST_SUCCESS) {
        // Disable all ports
        return XST_FAILURE;
    }
    if (response == knownValue) {
        // All good
        return XST_SUCCESS;
    }
    return XST_FAILURE;
...
int LEDdriver::regwrite(uint8_t reg,uint8_t data ) {
    ...
    regAddr[0] = reg;
    value[0] = data;
    return Audio::regwrite(regAddr,
        sizeof(regAddr), value, sizeof(value),
        IIC_MOTHERBOARD_PCA9956_ADDR,
        IIC_MOTHERBOARD_PCA9956_BUS
    );
}
...
uint8_t LEDdriver::regread() {
    ...
    uint8_t rx_data[1];
	int status = xiicps::master_recv_polled(&x[IIC_MOTHERBOARD_TCA9548A_BUS],
        rx_data,1, (IIC_MOTHERBOARD_PCA9956_ADDR & 0xff)
    );
	if (status != XST_SUCCESS) {
		return XST_FAILURE;
	}
...
int LEDdriver::initialize() {
    // XST_SUCCESS = 0, XST_FAILURE = 1
    int status = XST_SUCCESS;
    status |= LEDdriver::regwrite(MODE1, MODE1_SETTING_NO_INCREMENT);
    for (int i = 2; i < 8; i++) {
        status |= LEDdriver::regwrite(i, LEDMODE_FULLOFF);
    }
    // Use this if you want the same brightness for all the LEDs
    // (but check the maximum current of the differents led first! See above):
    ...
    status = xiicps::master_send_polled(&x[IIC_MOTHERBOARD_PCA9956_BUS],
        cmd, sizeof(cmd), IIC_MOTHERBOARD_PCA9956_ADDR
    );
...
void LEDdriver::allOn() {
    for (int i = 0; i < PCA9965_NUM_LEDS; i++) {
        LEDdriver::ledOn(i);
    }
}
...
void LEDdriver::allBlink() {
    for (int i = 0; i < PCA9965_NUM_LEDS; i++) {
         LEDdriver::ledBlink(i);
    }
}
...
void LEDdriver::saveState(uint8_t* savedState) {
    for (int i = 0; i < PCA9965_NUM_LEDS; i++) {
         savedState[i]=ledStatus[i];
    }
}

void LEDdriver::restoreState(uint8_t* savedState) {
    for (int i = 0; i < PCA9965_NUM_LEDS; i++) {
        ledStatus[i]=savedState[i];
    }
    for (int i = 0; i < 6; i++) {
        uint8_t LEDgroup = i;
        uint8_t regAdr = LEDgroup + LEDOUT0;
...
int motherBoard::initializeUI() {
    ...
    // Sample Rate LED
    if ((SYFALA_SAMPLE_RATE == 384000) | (SYFALA_SAMPLE_RATE == 768000)) {
        status |= LEDdriver::ledOn(20);
    }
    if ((SYFALA_SAMPLE_RATE == 48000) | (SYFALA_SAMPLE_RATE == 768000)) {
        status |= LEDdriver::ledOn(21);
    }
    // Bit Width LED
    if ((SYFALA_SAMPLE_WIDTH == 24) | (SYFALA_SAMPLE_WIDTH == 32)) {
        status |= LEDdriver::ledOn(22);
    }
    if ((SYFALA_SAMPLE_WIDTH == 16) | (SYFALA_SAMPLE_WIDTH == 32)) {
        status |= LEDdriver::ledOn(23);
    }
    return status;
}
...
void motherBoard::searchIIC() {
    ...
    for (int i = 0; i < 255; i++) {
         ...
         if (status != XST_SUCCESS) {
             debug("\033[2K[MotherBoard] No I2C at: 0x%02x \n\r", i);
         } else {
             debug("\033[2K[MotherBoard] I2C found at: 0x%02x \n\r", i);
         }
...
void motherBoard::printADAUTab(uint8_t *tab){
    ...
    for (int i=0; i<4; i++){
    info("|           |           |           |           |");
    info("|     %d     |     %d     |     %d     |     %d     |",tab[i+12],tab[i+8],tab[i+4],tab[i]);
    info("|           |           |           |           |");
    info("|0x%02x chD   |0x%02x chC   |0x%02x chB   |0x%02x chA   |",IIC_ADAU1787_SLAVE_ADDR_0+i,IIC_ADAU1787_SLAVE_ADDR_0+i,IIC_ADAU1787_SLAVE_ADDR_0+i,IIC_ADAU1787_SLAVE_ADDR_0+i);
    info("+-----------|-----------|-----------|-----------+");
    }
}

void motherBoard::ledCodecsAllOn() {
    for (int i = 0; i < 16; i++) {
         LEDdriver::ledOn(i);
    }
}
void motherBoard::ledCodecsAllOff() {
    for (int i = 0; i < 16; i++) {
         LEDdriver::ledOff(i);
    }
}
void motherBoard::ledCodecsAllBlink(){
    for (int i = 0; i < 16; i++) {
         LEDdriver::ledBlink(i);
    }
}
