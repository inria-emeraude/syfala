...
# FAUST IP
# -----------------------------------------------------------------------------
set faust [create_bd_cell -type "module" -reference "FAUST" FAUST_0]
...
switch $::rt::board {
Z10 - Z20 {
    source $::Syfala::SCRIPTS_DIR/bd_res/zybo_ps7.tcl
}
GENESYS {
    source $::Syfala::SCRIPTS_DIR/bd_res/genesys_psu.tcl
}
}
# -----------------------------------------------------------------------------
# Others
...
proc connect {args} {
    set cmd ""
    if {[lsearch -exact $args "intf_pins"]  >= 0 ||
        [lsearch -exact $args "intf_ports"] >= 0}  {
...
if {$::globals::clk_dynamic_reconfig} {
# ---------------------------------------------------
connect "intf_pins" clk_wiz_I2S/s_axi_lite          \
        "intf_pins" axi_periph_interconn/M03_AXI    \
# ---------------------------------------------------
connect "pins" clk_wiz_I2S/s_axi_aclk               \
        "pins" $system_clock
# ---------------------------------------------------
connect "pins" rst_sys_clk_125M/slowest_sync_clk    \
        "pins" $system_clock
# ---------------------------------------------------
connect "pins" rst_sys_clk_125M/ext_reset_in        \
        "pins" processing_system7_0/FCLK_RESET0_N
# ---------------------------------------------------
connect "pins" rst_sys_clk_125M/peripheral_aresetn  \
        "pins" clk_wiz_I2S/s_axi_aresetn
# ---------------------------------------------------
...
connect "pins" rst_global/slowest_sync_clk          \
        "pins" $i2s_clk_instance_name/clk_24Mhz
# ---------------------------------------------------

#foreach_n $::rt::nchannels_i {{n} {
# ---------------------------------------------------
connect "pins" FAUST_0/in_left_V                    \
        "pins" i2s_transceiver_0/to_faust_ch0
# ---------------------------------------------------
connect "pins" FAUST_0/in_right_V                   \
        "pins" i2s_transceiver_0/to_faust_ch1
#}}
# ---------------------------------------------------
connect "pins" FAUST_0/ws                           \
        "pins" i2s_transceiver_0/ws
# ---------------------------------------------------
#foreach_n $::rt::nchannels_o {{n} {
# ---------------------------------------------------
connect "pins" FAUST_0/out_left_V                   \
        "pins" i2s_transceiver_0/from_faust_ch0
# ---------------------------------------------------
connect "pins" FAUST_0/out_right_V                  \
        "pins" i2s_transceiver_0/from_faust_ch1
# ---------------------------------------------------
connect "pins" FAUST_0/out_left_V_ap_vld            \
        "pins" i2s_transceiver_0/from_faust_ch0_ap_vld
# ---------------------------------------------------
connect "pins" FAUST_0/out_right_V_ap_vld           \
        "pins" i2s_transceiver_0/from_faust_ch1_ap_vld
#}
# ---------------------------------------------------
#connect "ports" debug_btn                           \
#        "pins"  syfala/debug
# ---------------------------------------------------
connect "pins" FAUST_0/ap_done                      \
        "pins" i2s_transceiver_0/ap_done
# ---------------------------------------------------
connect "pins" FAUST_0/ap_start                     \
        "pins" i2s_transceiver_0/rdy
# ---------------------------------------------------
connect "pins" FAUST_0/bypass_dsp                   \
        "pins" sw0/Dout
# ---------------------------------------------------
connect "pins" FAUST_0/bypass_faust                 \
        "pins" sw1/Dout
# ---------------------------------------------------
#connect "pins" syfala/outGPIO_o                     \
...
if {$::rt::nchannels_o >= 2} {
#    connect "pins" syfala/audio_out_1_ap_vld "ports" syfala_out_debug2
}
...
connect "pins" FAUST_0/ap_rst_n            "pins" rst_global/peripheral_aresetn
...
connect "pins" $system_clock "pins" FAUST_0/ap_clk
...
if $::globals::clk_dynamic_reconfig {
    assign_bd_address -offset "0x44A00000" -range "0x00010000"              \
                      -target_address_space                                 \
                        ...
                        [get_bd_addr_segs "clk_wiz_I2S/s_axi_lite/Reg"]     \
                      -force
}
