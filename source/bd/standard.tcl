...
#PLEASE DON'T CHANGE THAT!! I know the only the name differ from ZYBO to GENESYS but Vivado NEED theses names!
...
# For switches and LED I now use the explicit declaration rather than the Xilinx interface to make it simpler between Zybo/Genesys. It uses exactly the same amount of resources.
...
#These are the names imposed by Xilinx which correspond to the boards interface. Don't change them!!
switch $::rt::board {
    Z10 - Z20 {
            set rgb_led_name "rgb_led"
            set greens_leds_name "leds_4bits"
        }
    GENESYS {
            set rgb_led_name "rgbled_3bits"
            set greens_leds_name "led_4bits"
        }
    }

set axi_gpio_LED [create_bd_cell -type "ip" -vlnv "xilinx.com:ip:axi_gpio:2.0" axi_gpio_LED]
set_property -dict [list \
    CONFIG.GPIO2_BOARD_INTERFACE $rgb_led_name \
    CONFIG.GPIO_BOARD_INTERFACE $greens_leds_name \
...
switch $::rt::board {
    Z10 - Z20 {
            set btn_name "btns_4bits"
            set sw_name "sws_4bits"
        }
    GENESYS {
            set btn_name "btn_5bits"
            set sw_name "switch_4bits"
        }
    }
set axi_gpio_SW [ create_bd_cell -type "ip" -vlnv "xilinx.com:ip:axi_gpio:2.0" axi_gpio_SW ]
set_property -dict [list \
    CONFIG.GPIO2_BOARD_INTERFACE $btn_name \
    CONFIG.GPIO_BOARD_INTERFACE $sw_name \
] $axi_gpio_SW
...
# SPI
# -----------------------------------------------------------------------------
connect	"pins" zynq_ultra_ps_e_0/emio_spi0_m_i    "ports" spi_MISO
