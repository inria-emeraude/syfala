...
monophonic(n,b) = index(n) : ba.parallelMax(N) : offset with {
    index(n)  = n : par(i, N, *(i+1));
    offset(x) = (x > 0) * b + x;
};

main_note = monophonic(pads,base);
main_gate = main_note > 0;
...
eei = mr2(vslider("[1] extInputOn [style:knob] [color:blue]",0,0,1,1)); // External input = MAIN OUTPUT when "off"
sei = mr2(vslider("[0] Ext Input [style: knob]",0,0,1.0,0.001));
...
amp2Enable = mr3(vslider("[1] amp2Enable [style:knob] [color:blue]",1,0,1,1));
...
noiseOff = mr4cbg(vslider("[0] noiseEnable [style:knob] [color:blue]",1,0,1,1));
...
vcfKeyRange = vcf1cbg(vslider("[2] Kbd Ctl [tooltip: Keyboard tracking of VCF corner-frequency (0=none, 1=full)]
		  [style: knob]",
		  1, 0, 1, 0.001)); // was in mr2
vcfModEnable = vcf1cbg(vslider("[1] Filter Mod. [midi:ctrl 34] [color:red] [style:knob] [tooltip: Filter Modulation => Route Modulation Mix output to VCF frequency]",1,0,1,1));
...
vcfContourAmountOctaves = vcf1(vslider("[3] Amount of Contour (octaves) [midi:ctrl 56][style: knob]", 1.2, 0, 4.0, 0.001));
...
attT60VCF = 0.001 * vcf2(vslider("[0] AttackF [tooltip: Attack Time] [unit:ms] [midi:ctrl 53] [style: knob]",1400,10,10000,1));
decT60VCF = 0.001 * vcf2(vslider("[0] DecayF  [tooltip: Decay-to-Sustain Time] [unit:ms] [midi:ctrl 54][style: knob]",10,10,10000,1));
susLvlVCF = 0.01 * vcf2(vslider("[0] SustainF [tooltip: Sustain level as percent of max][midi:ctrl 55][style: knob]",80,0,100,0.1));
decayButton = wg(vslider("Decay [tooltip:Envelope Release either Decay value or 0][style:knob]",1,0,1,1):int); // was Staccato
legatoButton = wg(vslider("GlideEnable [tooltip: Glide from note to note][style:knob]",1,0,1,1)); // was Legato
...
attT60 = 0.001 * ng(vslider("[0] AttackA [midi:ctrl 81] [tooltip: Attack Time] [unit:ms] [style: knob]",2,0,5000,0.1));
decT60 = 0.001 * ng(vslider("[0] DecayA [midi:ctrl 82] [tooltip: Decay-to-Sustain Time] [unit:ms] [style: knob]",10,0,10000,0.1));
susLvl = 0.01 * ng(vslider("[0] SustainA [midi:ctrl 83] [tooltip: Sustain level as percent of max] [style: knob]",80,0,100,0.1));
...
masterVolume = vg(vslider("MasterVolume [style:knob] [midi:ctrl 84] [tooltip: master volume, MIDI controlled]",
         0.7,0,1,0.001))
           : si.smooth(ba.tau2pole(0.16));
masterTuneOctaves = dg(vslider("[0] Tune [unit:Octaves] [style:knob]
...
glide = gmmg(vslider("[0] Glide [unit:sec/octave] [style:knob] [scale:log]
...
oscModEnable = dsg(vslider("[0] Osc. Mod. [color:red] [style:knob] [tooltip:Oscillator Modulation adds Modulation Mix output to osc1&2 frequencies",1,0,1,1)); // any offset?
osc3Control = dsg(vslider("[1] Osc. 3 Ctl [color:red] [style:knob] [tooltip:Oscillator 3 frequency tracks the keyboard if on, else not",0,0,1,1):int);
