...
harpe(N) = 	hand <: par(i, N, position((i+0.5)/N)
				 ...
				 :> _,_
	with {
		lvl  = hslider("level [unit:f][osc:/accxyz/0 -10 10][midi:ctrl 79]", 0.5, 0, 1, 0.01)^2;
		att  = hslider("attenuation [osc:/1/fader3][midi:ctrl 78]", 0.005, 0, 0.01, 0.001);
		hand = hslider("hand[osc:/accxyz/1 0 20][midi:ctrl 77]", 0, 0, 1, 0.01):smooth(0.9);
