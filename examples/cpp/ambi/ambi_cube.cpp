            ...
            if (bypass) {
                 ...
                 * (audio inputs and outputs are 24-bit integers) */
                 for (int n = 0; n < SYFALA_BLOCK_NSAMPLES; ++n) {
                     ...
                     float outs[OUTPUTS] = {0.0f};
                     ins[0] = Syfala::HLS::ioreadf(audio_in_0[n]);
                     ins[1] = Syfala::HLS::ioreadf(audio_in_1[n]);
                     ins[2] = Syfala::HLS::ioreadf(audio_in_2[n]);
                     ins[3] = Syfala::HLS::ioreadf(audio_in_3[n]);

                     for(int i = 0; i < INPUTS; i++){
                         // near field correction (NFC)
                         if(co[i] == 1){
                             ins[i] = ins[i]*nfc_g + nfc_del[i]*nfc_d1;
