...
float nfc1_d1;
float nfc1_g;
float nfc1_del[NFC1_NMAX][SYFALA_BLOCK_NSAMPLES];

float nfc2_d1;
float nfc2_d2;
float nfc2_g;
float nfc2_del[NFC2_NMAX*2][SYFALA_BLOCK_NSAMPLES];

float nfc3_d1;
float nfc3_d2;
float nfc3_d3;
float nfc3_g;
float nfc3_del[NFC3_NMAX*3][SYFALA_BLOCK_NSAMPLES];
...
float nfc4_d4;
float nfc4_g;
float nfc4_del[NFC4_NMAX*4][SYFALA_BLOCK_NSAMPLES];
        ...
        if (initialization) {
            ...
            if (bypass) {
                ...
                for(int i = 0; i < INPUTS; i++){
                    if(co[i] == 1){ // NFC 1
                        for (int n = 0; n < SYFALA_BLOCK_NSAMPLES; ++n) {
                            ins[i][n] = ins[i][n]*nfc1_g + nfc1_del[nfc1_n][n]*nfc1_d1;
                            nfc1_del[nfc1_n][n] = ins[i][n] + nfc1_del[nfc1_n][n];
                        }
                        nfc1_n++;
                    }
                    else if(co[i] == 2){ // NFC 2
                        for (int n = 0; n < SYFALA_BLOCK_NSAMPLES; ++n) {
                            ins[i][n] = ins[i][n]*nfc2_g + nfc2_del[nfc2_n][n]*nfc2_d1 + nfc2_del[nfc2_n+NFC2_NMAX][n]*nfc2_d2;
                            nfc2_del[nfc2_n][n] = ins[i][n] + nfc2_del[nfc2_n][n];
                            nfc2_del[nfc2_n+NFC2_NMAX][n] = nfc2_del[nfc2_n][n] + nfc2_del[nfc2_n+NFC2_NMAX][n];
                        }
                        nfc2_n++;
                    }
                    else if(co[i] == 3){ // NFC 3
                        for (int n = 0; n < SYFALA_BLOCK_NSAMPLES; ++n) {
                            ins[i][n] = ins[i][n]*nfc3_g + nfc3_del[nfc3_n][n]*nfc3_d1 + nfc3_del[nfc3_n+NFC3_NMAX][n]*nfc3_d2;
                            nfc3_del[nfc3_n][n] = ins[i][n] + nfc3_del[nfc3_n][n];
                            nfc3_del[nfc3_n+NFC3_NMAX][n] = nfc3_del[nfc3_n][n] + nfc3_del[nfc3_n+NFC3_NMAX][n];
                            ins[i][n] = ins[i][n] + nfc3_del[nfc3_n+NFC3_NMAX*2][n]*nfc3_d3;
                            nfc3_del[nfc3_n+NFC3_NMAX*2][n] = ins[i][n] + nfc3_del[nfc3_n+NFC3_NMAX*2][n];
                        }
                        nfc3_n++;
                    }
                    else if(co[i] == 3){ // NFC 4
                        for (int n = 0; n < SYFALA_BLOCK_NSAMPLES; ++n) {
                            ins[i][n] = ins[i][n]*nfc4_g + nfc4_del[nfc4_n][n]*nfc4_d1 + nfc4_del[nfc4_n+NFC4_NMAX][n]*nfc4_d2;
                            nfc4_del[nfc4_n+NFC4_NMAX][n] = ins[i][n] + nfc4_del[nfc4_n][n];
                            nfc4_del[nfc4_n+NFC4_NMAX][n] = nfc4_del[nfc4_n][n] + nfc4_del[nfc4_n+NFC4_NMAX][n];
                            ins[i][n] = ins[i][n] + nfc4_del[nfc4_n+NFC4_NMAX*2][n]*nfc4_d3 + nfc4_del[nfc4_n+NFC4_NMAX*3][n]*nfc4_d4;
                            nfc4_del[nfc4_n+NFC4_NMAX*2][n] = ins[i][n] + nfc4_del[nfc4_n+NFC4_NMAX*2][n];
                            nfc4_del[nfc4_n+NFC4_NMAX*3][n] = nfc4_del[nfc4_n+NFC4_NMAX*2][n] + nfc4_del[nfc4_n+NFC4_NMAX*3][n];
                        }
                        nfc4_n++;
                    }
