...
#pragma HLS INTERFACE m_axi port=out_samples bundle=ram latency=30 depth=SYFALA_SAMPLE_RATE
#pragma HLS INTERFACE m_axi port=mem_zone_i bundle=ram latency=30 depth=10
    ...
    if (arm_ok) {
        ...
        for (int m = 0 ; m < modesNumber; m++) {
            #pragma HLS performance target_tl=SYFALA_BLOCK_NSAMPLES*2000
