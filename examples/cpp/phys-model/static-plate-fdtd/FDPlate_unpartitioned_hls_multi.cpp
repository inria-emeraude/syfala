...
void process(bool trigger, f32 *output, f32 *u, f32 *u1, f32 *u2) {
    ...
    static f32 u1_cache[num_max_samples];
    static f32 u2_cache[num_max_samples];
    #pragma HLS array_partition variable=u1_cache type=cyclic factor=grid_height
    #pragma HLS array_partition variable=u2_cache type=cyclic factor=grid_height
    #pragma HLS bind_storage variable=u1_cache type=RAM_T2P impl=BRAM
    #pragma HLS bind_storage variable=u2_cache type=RAM_T2P impl=BRAM
    ...
    for (u32 row_index = 0; row_index < grid_height; row_index++) {
        ...
        for (u32 col_index = 0; col_index < grid_width; col_index++) {
            ...
            u1_cache[linear_index] = u1[linear_index];
            u2_cache[linear_index] = u2[linear_index];
    ...
    for (u32 row_index = 0; row_index < grid_height; row_index++) {
        ...
        for (u32 col_index = 0; col_index < grid_width; col_index++) {
            ...
            u1[linear_index] = u1_cache[linear_index];
            u2[linear_index] = u2_cache[linear_index];
        ...
        if (initialization) {
            ...
            if (bypass || mute) {
                ...
                for (u32 sample_index = 0; sample_index < SYFALA_BLOCK_NSAMPLES; sample_index++) {
                    ...
                    #ifdef GCC_STANDALONE
                    assert(abs(output) < 1.0 && "[HLS] output overflow");
