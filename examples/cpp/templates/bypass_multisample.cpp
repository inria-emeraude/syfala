        ...
        if (initialization) {
            ...
            if (bypass) {
                 ...
                 * (audio inputs and outputs are 24-bit integers) */
                 for (int n = 0; n < SYFALA_BLOCK_NSAMPLES; ++n) {
                      float f0 = Syfala::HLS::ioreadf(audio_in_0[n]);
                      float f1 = Syfala::HLS::ioreadf(audio_in_1[n]);
                      Syfala::HLS::iowritef(f0, &audio_out_0[n]);
                      Syfala::HLS::iowritef(f1, &audio_out_1[n]);
                 }
            }
        }
    }
